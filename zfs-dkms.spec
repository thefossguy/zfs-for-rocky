...
# taken from https://github.com/openzfs/zfs/releases/tag/zfs-%{version}
%define kernel_min 3.10
%define kernel_max 6.7.999
...
%if 0%{?rhel}%{?fedora}%{?mageia}%{?suse_version}%{?openEuler}
Requires:       kernel-devel >= %{kernel_min}, kernel-devel <= %{kernel_max}
Requires(post): kernel-devel >= %{kernel_min}, kernel-devel <= %{kernel_max}
